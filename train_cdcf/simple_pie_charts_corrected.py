...
class SimplePieChartAnalyzer:
    ...
    def plot_client_totals(self, save_path=None):
        """Plot pie charts showing total data distribution for each client (using fold 0 as representative)."""
        fig, axes = plt.subplots(1, self.num_clients, figsize=(6 * self.num_clients, 6))
        if self.num_clients == 1:
            axes = [axes]
        
        fig.suptitle('Label Distribution per Client (Total Data Available)', fontsize=16, fontweight='bold')
        
        for client_id in range(self.num_clients):
            ax = axes[client_id]
            
            # Use fold 0 to get the total data for this client (train + val from fold 0)
            # Test data is the same across all folds, so we can get it from any fold
            total_client_counts = {"BRS1": 0, "BRS2": 0, "BRS3": 0}
            test_counts = {"BRS1": 0, "BRS2": 0, "BRS3": 0}
            train_val_counts = {"BRS1": 0, "BRS2": 0, "BRS3": 0}
            
            if 0 in self.split_data[client_id]:  # Use fold 0 as representative
                df = self.split_data[client_id][0]
            ...
            # Create pie chart for total client data
            labels = ['BRS1', 'BRS2', 'BRS3']
            sizes = [total_client_counts[label] for label in labels]
            ...
            if total_samples > 0:
                ...
                for autotext in autotexts:
                    ...
                    client_total = (sum(total_train_counts.values()) + 
                                  sum(total_val_counts.values()) + 
                                  sum(total_test_counts.values()))
                    
                    ax.set_title(f'Client {client_id}\nTraining Data Distribution\nTotal Samples: {client_total}', 
                    ...
                    ax.text(0, -1.5, info_text, ha='center', va='top', fontsize=10, 
                           bbox=dict(boxstyle="round,pad=0.3", facecolor="lightgray", alpha=0.8))        plt.tight_layout()
        if save_path:
    ...
    def plot_client_folds_detailed(self, save_path=None):
        ...
        fig.suptitle('Label Distribution per Client and Fold (Training Data Only)', 
    ...
    def create_simple_visualizations(self, output_dir="simple_analysis"):
        ...
        # Plot 1: Client totals (corrected for k-fold CV)
...
def main():
    ...
    splits_dir = "/gris/gris-f/homelv/phempel/masterthesis/MM_flower/train_cdcf/splits/chimera_3_5_0.1_1_unbalanced_0.2_0.4_0.4"
