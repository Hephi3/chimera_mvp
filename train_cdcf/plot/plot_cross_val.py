...
def get_individual_fold_data(experiment_name, fold_numbers, metric, all_rounds, global_max_steps_per_round,
                            show_full_training, show_individual_clients, smooth_window=0):
    """
    Get individual fold data without aggregation for plotting separate curves
    ...
    Args:
        ...
        show_individual_clients: Whether to get data per client or aggregated
        smooth_window: Window size for smoothing (0 = no smoothing)
        
    Returns:
        Tuple of (fold_client_data, fold_server_data)
        fold_client_data: {fold_num: client_data}
        fold_server_data: {fold_num: server_data}
    """
    fold_client_data = {}
    fold_server_data = {}
    ...
    return fold_client_data, fold_server_data
...
def compute_fold_statistics(experiment_name, fold_numbers, metric, all_rounds, global_max_steps_per_round, 
    ...
    for round_num in all_rounds:
        ...
        # if "test" in metric:
            # print("For round", round_num, "and metric", metric, "server values:", round_values)
...
def plot_individual_folds_metric(ax, metric, title, fold_client_data, fold_server_data,
                                all_rounds, global_max_steps_per_round, 
                                show_full_training, show_individual_clients):
    """
    Plot individual fold curves (no aggregation)
    """
    LINE_ALPHA = 0.7
    
    # Colors for different folds
    fold_colors = ['blue', 'red', 'green', 'orange', 'purple', 'brown', 'pink', 'gray', 'cyan', 'magenta']
    
    # For train/val metrics, exclude round 0
    ...
    for fold_idx, (fold_num, client_data) in enumerate(sorted(fold_client_data.items())):
        ...
        if show_individual_clients:
            ...
            for client_id in sorted(client_data.keys()):
                if show_full_training:
                    ...
                    for round_num in plot_rounds:
                        if round_num in client_data[client_id] and metric in client_data[client_id][round_num]:
                            values = [v for v in client_data[client_id][round_num][metric] if v is not None]
                            if values:
                                round_values.append(values[-1])
            ...
            # Aggregate across clients for this fold
            if show_full_training:
                # Aggregate full training curves
                all_aggregated = []
                all_steps = []
                ...
                for round_idx, round_num in enumerate(plot_rounds):
                    ...
                    for client_id in sorted(client_data.keys()):
                        if round_num in client_data[client_id] and metric in client_data[client_id][round_num]:
                            values = [v for v in client_data[client_id][round_num][metric] if v is not None]
                            if values:
                                round_client_values.append(values)
                                max_steps = max(max_steps, len(values))
                    
                    if round_client_values and max_steps > 0:
                        # Average across clients for each step
                        round_avg = []
                        for step in range(max_steps):
                            step_values = []
                            for client_values in round_client_values:
                                if step < len(client_values):
                                    step_values.append(client_values[step])
                                else:
                                    step_values.append(client_values[-1])
                            if step_values:
                                round_avg.append(np.mean(step_values))
                        
                        if round_avg:
                            round_start = round_idx * global_max_steps_per_round
                            round_steps = list(range(round_start, round_start + len(round_avg)))
                            all_aggregated.extend(round_avg)
                            all_steps.extend(round_steps)
                
                if all_aggregated:
                    ax.plot(all_steps, all_aggregated, color=color, linewidth=2, 
                           label=f'Fold {fold_num}', alpha=LINE_ALPHA)
            else:
                ...
                for round_num in plot_rounds:
                    ...
                    for client_id in sorted(client_data.keys()):
                        if round_num in client_data[client_id] and metric in client_data[client_id][round_num]:
                            values = [v for v in client_data[client_id][round_num][metric] if v is not None]
                            if values:
                                round_final_values.append(values[-1])
                    
                    if round_final_values:
                        round_aggregated.append(np.mean(round_final_values))
                        round_positions.append(round_num)
                
                if round_aggregated:
                    ax.plot(round_positions, round_aggregated, color=color, marker='o',
                           linewidth=2, markersize=6, label=f'Fold {fold_num}', alpha=LINE_ALPHA)
    
    ax.set_title(title)
    ax.set_ylabel(title)
    ax.set_xlabel('Federated Round' if not show_full_training else 'Training Steps')
    ax.grid(True, alpha=0.3)


def plot_individual_folds_test_metric(ax, title, fold_client_data, fold_server_data,
                                     all_rounds, show_individual_clients):
    """
    Plot individual fold test metrics (no aggregation) with stage differentiation
    """
    LINE_ALPHA = 0.7
    ...
    # Plot each fold
    for fold_idx, fold_num in enumerate(sorted(fold_server_data[0].keys())):
        color = fold_colors[fold_idx % len(fold_colors)]
        
        # Plot both stages for this fold
        for stage in [0, 1]:
            if fold_num not in fold_server_data[stage]:
                continue
                
            server_data = fold_server_data[stage][fold_num]
            linestyle = stage_linestyles[stage]
            marker = stage_markers[stage]
            
            # Plot server evaluation for this fold
            server_values = []
            server_rounds = []
            
            for round_num in sorted(server_data.keys()):
                if round_num in plot_rounds:
                    # Get the metric value from server_data
                    for metric_key, metric_value in server_data[round_num].items():
                        server_values.append(metric_value)
                        server_rounds.append(round_num)
                        break  # Only take the first metric (should be the one we want)
            
            if server_values:
                ax.plot(server_rounds, server_values, color=color, marker=marker, linestyle=linestyle,
                       linewidth=2, markersize=6, label=f'Fold {fold_num} Stage {stage}', alpha=LINE_ALPHA)
...
def plot_crossval_test_metric(ax, title, client_stats, server_stats, all_rounds, 
                        show_individual_clients, 
    ...
    if show_individual_clients and client_stats:
        ...
        for i in [0,1]:
            for client_idx, client_id in enumerate(sorted(client_stats[i].keys())):
                # Use different shades/styles for different stagees
                if i == 0:
                    color = base_client_colors[client_idx % len(base_client_colors)]
                    linestyle = '--'
                else:
                    # For stage 1, use similar colors but with different line style
                    color = base_client_colors[client_idx % len(base_client_colors)]
                    linestyle = ':'
                ...
                for round_num in sorted(client_stats[i][client_id].keys()):
                    if round_num in plot_rounds:
                        mean_val = client_stats[i][client_id][round_num]['mean']
                        std_val = client_stats[i][client_id][round_num]['std']
    ...
    if server_stats:
        ...
        stage_colors = ['black', 'red']  # Stage 0: black, Stage 1: red
        ...
        for i in [0,1]:
            ...
            for round_num in sorted(server_stats[i].keys()):
                if round_num in plot_rounds:
                    server_means.append(server_stats[i][round_num]['mean'])
                    server_stds.append(server_stats[i][round_num]['std'])
                    server_rounds.append(round_num)
            
            if server_means:
                # Plot mean line - use different colors for each stage
                server_color = stage_colors[i] if show_individual_clients and client_stats else ('blue' if i == 0 else 'red')
                server_marker = 's' if show_individual_clients and client_stats else 'o'
                server_label = f'Global Model Stage {i} (CV avg)' if show_individual_clients and client_stats else f'Cross-Val Average Stage {i}'
...
def plot_crossval_metric(ax, metric, title, client_stats, all_rounds, 
...
def create_legend_subplot(axs, ax_dims, ax_width, metrics_to_plot, show_individual_clients=False, client_ids=None):
    ...
    for i, (base_metric, title) in enumerate(metrics_to_plot.items()):
        if base_metric == 'Legend':
            ...
            # Check if we have test metrics with stagees
            ...
            if show_individual_clients and client_ids:
                ...
                if test_metrics_exist:
                    # Add client legend entries for both stagees
                    for client_idx, client_id in enumerate(sorted(client_ids)):
                        color = base_client_colors[client_idx % len(base_client_colors)]
                        # Stage 0 with dashed line
                        line_0 = plt.Line2D([0], [0], color=color, linewidth=2, linestyle='--',
                                           label=f'Client {client_id} Stage 0')
                        legend_handles.append(line_0)
                        # Stage 1 with dotted line
                        line_1 = plt.Line2D([0], [0], color=color, linewidth=2, linestyle=':',
                                           label=f'Client {client_id} Stage 1')
                        legend_handles.append(line_1)
                else:
                    # Add client legend entries for non-test metrics
                    for client_idx, client_id in enumerate(sorted(client_ids)):
                        color = base_client_colors[client_idx % len(base_client_colors)]
                        line = plt.Line2D([0], [0], color=color, linewidth=2, linestyle='-',
                                         label=f'Client {client_id}')
                        legend_handles.append(line)
            
            # Add server/global model legend
            if test_metrics_exist:
                # Add legend for both stagees
                stage_colors = ['black', 'red']
                server_marker = 's' if show_individual_clients and client_ids else 'o'
                for stage_idx in [0, 1]:
                    server_color = stage_colors[stage_idx] if show_individual_clients and client_ids else ('blue' if stage_idx == 0 else 'red')
                    server_label = f'Global Model Stage {stage_idx} (CV avg)' if show_individual_clients and client_ids else f'Cross-Val Average Stage {stage_idx}'
                ...
                # Single legend entry for non-test metrics
...
def plot_crossval_test_metric_comparison(ax, title, all_experiment_data, all_experiment_stats, 
                                       base_metric, all_rounds, show_individual_clients, 
                                       show_std, exp_colors, show_legend=True):
    """
    Plot test metrics comparison with both stages across multiple experiments
    """
    LINE_ALPHA = 0.8
    STD_ALPHA = 0.12
    plot_rounds = all_rounds
    
    # Define line styles for different stages
    ...
    if show_individual_clients:
        ...
        for exp_idx, (exp_name, fold_numbers) in enumerate(all_experiment_data.items()):
            if exp_name not in all_experiment_stats or base_metric not in all_experiment_stats[exp_name]:
                continue
                
            client_stats, server_stats = all_experiment_stats[exp_name][base_metric]
            exp_color = exp_colors[exp_idx % len(exp_colors)]
            
            for stage in [0, 1]:
                if stage not in client_stats:
                    continue
                
                linestyle = stage_linestyles[stage]
                    
                for client_idx, client_id in enumerate(sorted(client_stats[stage].keys())):
                    ...
                    for round_num in sorted(client_stats[stage][client_id].keys()):
                        if round_num in plot_rounds:
                            mean_val = client_stats[stage][client_id][round_num]['mean']
                            std_val = client_stats[stage][client_id][round_num]['std']
                    ...
                    if client_means:
                        ...
                        if show_std and client_stds and any(std > 0 for std in client_stds):
                            ...
                            ax.fill_between(client_rounds, 
                                            client_means_array - client_stds_array, 
                                            client_means_array + client_stds_array,
                                            color=exp_color, alpha=STD_ALPHA * 0.9)
    ...
    for exp_idx, (exp_name, fold_numbers) in enumerate(all_experiment_data.items()):
        ...
        for stage in [0, 1]:
            ...
            for round_num in sorted(server_stats[stage].keys()):
                if round_num in plot_rounds:
                    server_means.append(server_stats[stage][round_num]['mean'])
                    server_stds.append(server_stats[stage][round_num]['std'])
                    server_rounds.append(round_num)
            
            if server_means:
                # Plot mean line - use same color but different line styles and markers for different stages
                server_label = f'{exp_name} Global Stage {stage}' if show_individual_clients else f'{exp_name} Stage {stage}'
                ax.plot(server_rounds, server_means, color=exp_color, marker=marker, linestyle=linestyle,
                        linewidth=2, markersize=6, label=server_label, alpha=LINE_ALPHA)
                ...
                if show_std and any(std > 0 for std in server_stds):
                    ...
                    ax.fill_between(server_rounds, 
                                    server_means - server_stds, 
                                    server_means + server_stds,
                                    color=exp_color, alpha=STD_ALPHA)
    ...
    #     handles, labels = ax.get_legend_handles_labels()
    #     if handles:
    #         # Sort legend items by experiment name and stage for better organization
    #         sorted_items = sorted(zip(labels, handles), key=lambda x: (x[0].split()[0], 'Stage 0' in x[0]))
    #         sorted_labels, sorted_handles = zip(*sorted_items) if sorted_items else ([], [])
    #         ax.legend(sorted_handles, sorted_labels, fontsize=8, ncol=2 if len(sorted_labels) > 6 else 1)
...
def plot_cross_validation_comparison(experiment_names: list, submodel: str = 'MM', metric_filter: str = 'test',
    ...
    for exp_name, fold_numbers in all_experiment_data.items():
        ...
        for base_metric in metrics_to_plot.keys():
            ...
            if not "test" in base_metric:
                ...
                all_experiment_stats[exp_name][base_metric] = (client_stats, server_stats)
            else:
                # Handle test metrics with both stages
                all_server_stats = {}
                all_client_stats = {}
                for stage in [0, 1]:
                    metric = f"{base_metric}/{submodel}/{stage}"
                    client_stats, server_stats = compute_fold_statistics(
                        exp_name, fold_numbers, metric, global_all_rounds, global_max_steps_per_round,
                        show_full_training, show_individual_clients, smooth_window
                    )
                    all_server_stats[stage] = server_stats
                    all_client_stats[stage] = client_stats
                
                all_experiment_stats[exp_name][base_metric] = (all_client_stats, all_server_stats)
    ...
    for i, (base_metric, title) in enumerate(metrics_to_plot.items()):
        ...
        if not "test" in base_metric:
            # Handle non-test metrics (train/val)
            ...
            # Handle test metrics with both stages
            # We need to create a custom plotting approach for comparison with stages
            # For test metrics, always show legend because we need to differentiate stages
            plot_crossval_test_metric_comparison(ax, title, all_experiment_data, all_experiment_stats, 
                                                base_metric, global_all_rounds, show_individual_clients, 
                                                show_std, exp_colors, show_legend=True)
    
    # Create comparison legend if needed
    if has_legend_entry:
        comparison_legend_handles = []
        # Add two legend entries per experiment: Stage 0 (solid) and Stage 1 (dashed)
        stage_linestyles = ['-', '--']
        stage_markers = ['s', '^']
        for exp_idx, exp_name in enumerate(all_experiment_data.keys()):
            color = exp_colors[exp_idx % len(exp_colors)]
            # Stage 0 handle
            line0 = plt.Line2D([0], [0], color=color, linewidth=2, linestyle=stage_linestyles[0],
                               marker=stage_markers[0], markersize=6, label=f'{exp_name} Stage 0')
            comparison_legend_handles.append(line0)
            # Stage 1 handle
            line1 = plt.Line2D([0], [0], color=color, linewidth=2, linestyle=stage_linestyles[1],
                               marker=stage_markers[1], markersize=6, label=f'{exp_name} Stage 1')
...
def plot_cross_validation(experiment_name: str, submodel: str = 'MM', metric_filter: str = 'test',
                         show_individual_clients: bool = False, show_full_training: bool = False,
                         smooth_window: int = 0, show_std: bool = True, folds: list = None,
                         show_individual_folds: bool = False):
    """
    ...
    Args:
        ...
        show_std: If True, show standard deviation as bands/error bars
        folds: List of specific fold numbers to include (None = use all available)
        show_individual_folds: If True, show individual fold curves instead of cross-validation aggregation
    """
    ...
    if show_individual_folds:
        mode_desc.append("individual folds")
    elif show_individual_clients:
        mode_desc.append("individual clients")
    else:
        mode_desc.append("aggregated clients")
    if show_full_training:
    ...
    if smooth_window > 0:
        mode_desc.append(f"smoothed (w={smooth_window})")
    if show_std and not show_individual_folds:
        mode_desc.append("with std dev")
    ...
    for i, (base_metric, title) in enumerate(metrics_to_plot.items()):
        ...
        if show_individual_folds:
            # Plot individual folds instead of cross-validation statistics
            if not "test" in base_metric:
                metric = f"{base_metric}/{submodel}"
                fold_client_data, fold_server_data = get_individual_fold_data(
                    experiment_name, fold_numbers, metric, global_all_rounds, global_max_steps_per_round,
                    show_full_training, show_individual_clients, smooth_window
                )
                plot_individual_folds_metric(ax, metric, title, fold_client_data, fold_server_data, 
                                           global_all_rounds, global_max_steps_per_round, 
                                           show_full_training, show_individual_clients)
            else:
                all_fold_client_data = {}
                all_fold_server_data = {}
                for stage in [0, 1]:
                    metric = f"{base_metric}/{submodel}/{stage}"
                    fold_client_data, fold_server_data = get_individual_fold_data(
                        experiment_name, fold_numbers, metric, global_all_rounds, global_max_steps_per_round,
                    ...
                    all_fold_client_data[stage] = fold_client_data
                    all_fold_server_data[stage] = fold_server_data
                plot_individual_folds_test_metric(ax, title, all_fold_client_data, all_fold_server_data,
                                                 global_all_rounds, show_individual_clients)
        else:
            # Original cross-validation aggregation
            if not "test" in base_metric:
                metric = f"{base_metric}/{submodel}"
                ...
                plot_crossval_metric(ax, metric, title, client_stats, global_all_rounds,
                               global_max_steps_per_round, show_full_training, show_individual_clients,
                ...
                for stage in [0, 1]:
                    ...
                    all_server_stats[stage] = server_stats
                    all_client_stats[stage] = client_stats
                plot_crossval_test_metric(ax, title, all_client_stats, all_server_stats, global_all_rounds,
    ...
    suffix_parts = []
    if show_individual_folds:
        suffix_parts.append("individual_folds")
    elif show_individual_clients:
        suffix_parts.append("individual")
    else:
        suffix_parts.append("aggregated")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--show_individual_folds', action='store_true',
                       help='Show individual fold curves instead of cross-validation aggregation')
    ...
    if args.compare and args.names:
        ...
        plot_cross_validation(args.name, args.submodel, args.metric_filter,
                             args.show_individual_clients, args.show_full_training,
                             args.smooth_window, show_std, args.folds, args.show_individual_folds)
    else:
...
# Show individual folds instead of cross-validation aggregation:
# python plot_cross_val.py --name del1_s1 --submodel MM --metric_filter test --show_individual_folds

# Show individual folds with specific fold numbers:
# python plot_cross_val.py --name del1_s1 --submodel MM --metric_filter all --show_individual_folds --folds 0 1 2
