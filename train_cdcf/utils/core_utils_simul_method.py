...
def train(model, train_split, val_split, args, cur, device, round_num=None, use_phases = True):
    ...
    assert args.max_epochs == 1, "Only 1 epoch supported in FL setting!"
    ...
    for epoch in tqdm(range(args.max_epochs), desc='Training Epochs', disable=not verbose):
        ...
        train_loss_mm, f1, features_list, labels_list = train_loop_clam(epoch, model, train_loader, optimizer,args.bag_weight, writer, loss_fn, verbose=verbose, phase=phase, device=device)
    ...
    return train_loss_mm, f1, features_list, labels_list
