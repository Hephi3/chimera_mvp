...
def train(datasets, cur, args, device):
    ...
    for epoch in tqdm(range(args.max_epochs), desc='Training Epochs'):#, disable=not verbose):
        ...
        train_loop_clam(epoch, model, train_loader, optimizer,args.bag_weight, writer, loss_fn, verbose=verbose, phase=phase, device=device, args=args)
...
def train_loop_clam(epoch, model, loader, optimizer, bag_weight, writer = None, loss_fn = None, verbose = True, phase = None, device=None, args=None):
    ...
    loss_weights_log_path = os.path.join(args.results_dir, "loss_weights_epochs.txt")
    os.makedirs(os.path.dirname(loss_weights_log_path), exist_ok=True)
    with open(loss_weights_log_path, 'a') as f:
        f.write(f"{w_mm}, {w_clam}, {w_cd}\n")
