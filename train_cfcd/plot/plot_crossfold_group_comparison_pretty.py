...
def plot_crossfold_group_test_metric(ax, metric, title, client_stats, server_stats, all_rounds, 
    ...
    if server_stats:
        ...
        if stage_0_means:
            ...
            ax.plot(stage_0_rounds, stage_0_means, color=color, 
        ...
        if stage_1_means:
            ...
            ax.plot(stage_1_rounds, stage_1_means, color=color, 
                   linewidth=2, markersize=6, label=label, alpha=LINE_ALPHA, linestyle='--')
    ...
    if has_legend_entry:
        ...
        if has_test_metrics:
            ...
            for group_idx, group_name in enumerate(all_group_data.keys()):
                color = group_colors[group_idx % len(group_colors)]
                # Stage 0 entry
                line0 = plt.Line2D([0], [0], color=color, linewidth=2, linestyle='-',
                                  label=f'{group_name}')
                comparison_legend_handles.append(line0)
                # Stage 1 entry
                # line1 = plt.Line2D([0], [0], color=color, linewidth=2, linestyle='--',
                #                   label=f'{group_name} Stage 1')
                # comparison_legend_handles.append(line1)
            
            line0 = plt.Line2D([0], [0], color="black", linewidth=2, linestyle='-',
                                label=f'Stage 1')
            comparison_legend_handles.append(line0)
            # Stage 1 entry
            line1 = plt.Line2D([0], [0], color="black", linewidth=2, linestyle='--',
                                label=f'Stage 2')
            comparison_legend_handles.append(line1)
        else:
