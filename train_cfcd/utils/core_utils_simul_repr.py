...
def log_metrics(writer, epoch, loss, all_labels, all_preds, all_probs, kind:str, submodel:str, stage:int=None):
    ...
    if writer is not None:
        if kind == 'test':
            assert stage is not None, "Stage must be provided for test logging"
            if loss is not None: writer.add_scalar(f'Loss/{kind}/{submodel}/{stage}', loss, epoch)
            writer.add_scalar(f'Accuracy/{kind}/{submodel}/{stage}', acc, epoch)
            writer.add_scalar(f'ROC_AUC/{kind}/{submodel}/{stage}', roc_auc, epoch) # Binary
            writer.add_scalar(f'F1/{kind}/{submodel}/{stage}', f1, epoch) # Binary
            writer.add_scalar(f'Binary_Accuracy/{kind}/{submodel}/{stage}', binary_acc, epoch) # Binary
        else:
...
# def train(datasets, cur, args, device):
    ...
    if type(client_nr) == int:
        ...
        writer_dir = os.path.join(writer_dir, f"{client_nr}_round_{round_nr}")
    if not os.path.isdir(writer_dir):
        os.mkdir(writer_dir)
    return writer_dir
...
def train(model, train_split, val_split, args, cur, device, round_num=None, use_phases = True, prototype=None, brs_samples=None, no_training=False):
    ...
    for epoch in tqdm(range(args.max_epochs), desc='Training Epochs', disable=not verbose):
        ...
        train_loss, f1, repr_list, labels_list = train_loop_clam(epoch, model, train_loader, optimizer,args.bag_weight, writer, loss_fn, verbose=verbose, phase=phase, device=device, prototype=prototype, brs_samples=brs_samples, no_training=no_training, strictness=args.strictness, args=args)
    ...
    return train_loss, f1, repr_list, labels_list
...
def train_loop_clam(epoch, model, loader, optimizer, bag_weight, writer = None, loss_fn = None, verbose = True, phase = None, device=None, prototype:PrototypeRepr=None, brs_samples:list[list]=None, no_training=False, strictness=1.0, args=None):
    ...
    repr_list = [[],[],[],[]]  # Clinical, scale 0, scale 1, scale 2
    ...
    return train_loss_mm, f1, repr_list, all_labels
...
def test(model, test_split, args, device, results_dir=None, client_nr=None, n_classes=3, round_nr=None, stage=None):
    if args.bag_loss == 'svm':
        from topk.svm import SmoothTop1SVM
        loss_fn = SmoothTop1SVM(n_classes = args.n_classes)
        if device.type == 'cuda':
    ...
    loss, f1 = test_clam(model, test_loader, device=device, args=args, results_dir=results_dir, client_nr=client_nr, n_classes=n_classes, loss_fn=loss_fn, round_nr=round_nr, stage=stage)
    return loss, f1

def test_clam(model, loader, device, args, results_dir=None, client_nr=None, n_classes=3, loss_fn=None, round_nr=None, stage=None):
    model.eval()
    test_loss = 0.
    test_error = 0.
    ...
    acc, roc_auc, f1 = log_metrics(writer, None, None, all_labels, all_preds, all_probs, 'test', 'MM', stage=stage)
    writer.flush()  # Ensure MM metrics are written

    log_metrics(writer, None, None, all_labels, all_preds_clam, all_probs_clam, 'test', 'CLAM', stage=stage)
    writer.flush()  # Ensure CLAM metrics are written
    
    # Only log CD metrics if we have valid CD results
    if has_cd_results:
        log_metrics(writer, None, None, all_labels, all_preds_cd, all_probs_cd, 'test', 'CD', stage=stage)
