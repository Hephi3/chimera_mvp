...
def get_split_by_class(dataset:Dataset, df_slice, data_dir, num_classes, page, **kwargs):
    if dataset.__class__.__name__ == 'Generic_MIL_Dataset':
        ...
        split = MM_Multi_Scale_Split(df_slice, data_dir=data_dir, num_classes=num_classes, page=page, return_coords = kwargs.get('return_coords', False), pages = kwargs.get('pages', False), no_cd_aug=kwargs.get('no_cd_aug', False))
...
class MM_Multi_Scale_Dataset(Generic_Multi_Scale_MIL_Dataset):
    def __init__(self, **kwargs):
        ...
        self.clinical_data = load_clinical_data()
        self.no_cd_aug = kwargs.get('no_cd_aug', False)
    ...
    def __getitem__(self, idx):
        ...
        if self.no_cd_aug:
            clinical_features = [0] * 23 
            print("Return zero clinical features for augmented data")
        else:
            clinical_features = self.clinical_data.get(slide_id, None)
        clinical_features = torch.tensor(clinical_features, dtype=torch.float32) if clinical_features is not None else None
...
class MM_Multi_Scale_Split(MM_Multi_Scale_Dataset):
    def __init__(self, slide_data, data_dir=None, num_classes=2, page=0, return_coords=False, pages = None, no_cd_aug=False):
        self.clinical_data = load_clinical_data()
        self.page = page
        self.use_h5 = False
        self.slide_data = slide_data
