...
def load_weights_from_file(weights_file):
    """
    Load prototype weights from file.
    
    Args:
        weights_file: Path to the prototype_weights.txt file
        
    Returns:
        Dictionary mapping slide_id to weight
    """
    weights_by_slide = {}
        ...
        with open(weights_file, 'r') as f:
            for line in f:
                ...
                try:
                    ...
                    weights_by_slide[slide_id] = weight
                except ValueError:
                    continue
...
def plot_prototype_evolution(samples_per_round, num_clients=3, client_colors=None, show_samples=True, sample_alpha=0.05, 
    ...
    Args:
        ...
        weights_by_slide: Dictionary mapping slide_id to list of weights (optional)
        slide_ids_per_round: List of lists, where slide_ids_per_round[round][client] is the slide IDs for that client at that round
    """
    ...
    # Setup colormap for weights if available
    use_weights = weights_by_slide is not None and slide_ids_per_round is not None
    if use_weights:
        # Collect all weights to determine colormap range
        all_weights = list(weights_by_slide.values())
        if all_weights:
            vmin, vmax = min(all_weights), max(all_weights)
            norm = mcolors.Normalize(vmin=vmin, vmax=vmax)
            cmap = cm.get_cmap('RdYlGn')  # Red (low weight) to Green (high weight)
        else:
            use_weights = False
    ...
    # Debug: Check weight matching
    if use_weights:
        print(f"\nWeight matching debug:")
        print(f"Total unique slides in weights file: {len(weights_by_slide)}")
        print(f"Sample weight keys: {list(weights_by_slide.keys())[:5]}")
        if slide_ids_per_round:
            print(f"Sample slide IDs from round 0, client 0: {slide_ids_per_round[0][0][:5]}")
            # Check for matches
            matched = 0
            unmatched = 0
            for slide_id in slide_ids_per_round[0][0]:
                if str(slide_id) in weights_by_slide:
                    matched += 1
                else:
                    unmatched += 1
    ...
    if show_samples:
        for round_idx, round_samples in enumerate(samples_per_round):
            ...
            for client_idx in range(num_clients):
                ...
                if use_weights and round_idx < len(slide_ids_per_round):
                    # Get slide IDs for this client at this round
                    slide_ids = slide_ids_per_round[round_idx][client_idx]
                    
                    # Get weights for each sample
                    colors_for_batch = []
                    for sample_idx, (point_2d, slide_id) in enumerate(zip(samples_2d, slide_ids)):
                        # Convert slide_id to string to ensure matching
                        slide_id_str = str(slide_id)
                        if slide_id_str in weights_by_slide:
                            weight = weights_by_slide[slide_id_str]
                            color = cmap(norm(weight))
                        else:
                            # Fallback to gray for unmatched samples
                            color = (0.5, 0.5, 0.5, 1.0)
                        
                        ax.scatter(point_2d[0], point_2d[1], 
                                 c=[color], s=round_size, alpha=round_alpha, 
                                 edgecolors='none', zorder=0)
                else:
                    # Plot samples as dots with client color
    ...
    # Add colorbar if using weights
    if use_weights:
        sm = cm.ScalarMappable(cmap=cmap, norm=norm)
        sm.set_array([])
        cbar = plt.colorbar(sm, ax=ax)
        cbar.set_label('Prototype Weight', fontsize=14)
    
    plt.tight_layout()
    # plt.show()
    # Instead store and print in console where it has been stored
    
    plt.savefig("prototype_evolution.png", dpi=150)
...
def load_features_from_rounds(base_dir, num_clients=3, rounds=None):
    ...
    Returns:
        ...
        slide_ids_per_round: List of lists [round][client] -> slide IDs
    """
    if rounds is None:
    ...
    for round_num in rounds:
        ...
        for client_id in range(num_clients):
            ...
            try:
                ...
                if round_num == rounds[0] and client_id == 0:
                ...
                # Try to load slide IDs if available
                if "slide_ids" in data:
                    slide_ids = data["slide_ids"]
                    # Convert to strings to ensure consistent matching
                    slide_ids = [str(sid) for sid in slide_ids]
                    if round_num == rounds[0] and client_id == 0:
                        print(f"Loaded slide_ids, length: {len(slide_ids)}")
                        print(f"Raw slide_ids first element type: {type(slide_ids[0])}")
                        print(f"First 5 converted slide_ids: {slide_ids[:5]}")
                    round_slide_ids.append(slide_ids)
                else:
                    if round_num == rounds[0] and client_id == 0:
                        print("No 'slide_ids' key found in NPZ file")
                    round_slide_ids.append([f"unknown_{i}" for i in range(len(features))])
                ...
                if len(round_samples) > 0:
                    ...
                    round_slide_ids.append(round_slide_ids[-1] if round_slide_ids else [])
                else:
        ...
        if len(round_samples) == num_clients:
            ...
            slide_ids_per_round.append(round_slide_ids)
    
    return samples_per_round, slide_ids_per_round
...
if __name__ == "__main__":
    ...
    # base_dir = "/gris/gris-f/homelv/phempel/masterthesis/MM_flower/train_cfcd/results/73_ML_02_str1_sp1_s3/Fold0"
    base_dir = "/gris/gris-f/homelv/phempel/masterthesis/MM_flower/train_cfcd/results/73_ML_02_2str_slideids_sp1_s3/Fold0"
    rounds_to_plot = list(range(1, 41))  # Rounds 1-40
    
    # Load weights if available
    weights_file = f"{base_dir}/prototype_weights.txt"
    weights_by_slide = load_weights_from_file(weights_file)
    
    samples_per_round, slide_ids_per_round = load_features_from_rounds(base_dir, num_clients=3, rounds=rounds_to_plot)
    if len(samples_per_round) > 0:
        print(f"Loaded {len(samples_per_round)} rounds")
        if weights_by_slide:
            print(f"Loaded weights for {len(weights_by_slide)} unique slides")
            plot_prototype_evolution(samples_per_round, num_clients=3, show_samples=True, sample_alpha=0.4,
                                   weights_by_slide=weights_by_slide, slide_ids_per_round=slide_ids_per_round)
        else:
            print("No weights loaded, using default client colors")
            plot_prototype_evolution(samples_per_round, num_clients=3, show_samples=True, sample_alpha=0.4)
    else:
