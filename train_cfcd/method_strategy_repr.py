    ...
    def aggregate_fit(self, server_round: int, results: list[tuple[ClientProxy | FitRes]], failures: list[tuple[ClientProxy, FitRes] | BaseException]) -> tuple[Parameters | None, dict[str,bool |bytes | float | int | str]]:
        if self.hyps.num_sampled > 0:
            ...
            for client_proxy, fit_res in results:
                ...
                brs_prototypes[0][id] = PrototypeRepr.deserialize(fit_res.metrics.get("prototype_brs1", None))
                brs_prototypes[1][id] = PrototypeRepr.deserialize(fit_res.metrics.get("prototype_brs2", None))
                brs_prototypes[2][id] = PrototypeRepr.deserialize(fit_res.metrics.get("prototype_brs3", None))

            for i in [1,2,3]:
                global_brs_prototype = PrototypeRepr.from_prototypes(brs_prototypes[i-1])
        ...
        for client_proxy, fit_res in results:
            ...
            if proto is not None:
                prototype = PrototypeRepr.deserialize(proto)
        ...
        new_global_prototype = PrototypeRepr.from_prototypes(prototypes, plot=False)
        if server_round == 1:
        ...
        new_global_prototype = PrototypeRepr.from_prototypes(prototypes, weights=normed_weights, plot=True)
